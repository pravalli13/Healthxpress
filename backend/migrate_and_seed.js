...
async function migrateAndSeed() {
  console.log('🔄 Re-aligning MySQL Tables with Normalized ABDM Schema...');
  const conn = await pool.getConnection();

  try {
    // Disable foreign key checks to recreate cleanly
    await conn.query('SET FOREIGN_KEY_CHECKS = 0');
    
    const tablesToDrop = [
      'audit_logs', 'qr_consent_tokens', 'ai_sessions', 'health_records',
      'prescriptions', 'appointments', 'doctor_schedules', 'doctor_hospitals',
      'doctors', 'departments', 'hospitals', 'health_profiles', 'users', 'medicines', 'tickets', 'payments'
    ];
    ...
    console.log('✅ Previous legacy tables cleared.');
    ...
    // 12. QR Consent Tokens
    await conn.query(`
      CREATE TABLE qr_consent_tokens (
        id VARCHAR(64) PRIMARY KEY,
        user_id VARCHAR(64) NOT NULL,
    ...
    console.log('✅ All 16 normalized tables created successfully.');

    // Now Seed Real Production Data
    console.log('\nInserting real hospitals data...');
    const hospitals = [
      {
        id: 'HOSP-01',
        name: 'KIMS Hospitals',
    ...
    for (const d of depts) {
      await conn.query('INSERT INTO departments (id, hospital_id, name, head_doctor_name) VALUES (?, ?, ?, ?)', [d.id, d.hospital_id, d.name, d.head_doctor_name]);
    }
    ...
    // Seed Patients & Aarogyasri Health Profiles
    console.log('Inserting verified patients and health records...');
    const users = [
      {
        id: 'USR-101',
        name: 'Rahul Kumar',
    ...
    for (const u of users) {
      await conn.query(
        `INSERT INTO users (id, name, mobile, email, role, is_profile_completed) VALUES (?, ?, ?, ?, 'user', TRUE)`,
    ...
    // Seed Real Appointments
    console.log('Inserting verified appointment queue...');
    await conn.query(`
      ...
      ('BK24851', 'USR-101', 'DOC-1024', 'HOSP-01', 'In-Clinic', CURDATE(), '10:30 AM', 800.00, 'paid', 'accepted', 'confirmed', TRUE, 'AROG12345678', 'Mild chest tightness, routine review', 'HEAL-ROOM-101'),
      ('BK24850', 'USR-102', 'DOC-1025', 'HOSP-02', 'In-Clinic', CURDATE(), '12:00 PM', 600.00, 'paid', 'accepted', 'confirmed', TRUE, 'AROG88900112', 'Follow-up blood pressure review', 'HEAL-ROOM-102'),
      ('BK24849', 'USR-103', 'DOC-1026', 'HOSP-03', 'Video', CURDATE(), '02:30 PM', 700.00, 'paid', 'accepted', 'confirmed', TRUE, 'AROG77865544', 'High fever and knee discomfort', 'HEAL-ROOM-103');
    `);

    // Seed Medicines Catalog
    console.log('Inserting verified pharmacy items...');
    const medicines = [
    ...
    conn.release();
    console.log('\n================================================================');
    console.log('🎉 REAL PRODUCTION DATA SEEDED AND READY ON 147.93.101.73:3306!');
    console.log('================================================================');
  } catch (err) {
    conn.release();
